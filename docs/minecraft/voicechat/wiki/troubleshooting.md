...
If your port is opened to an IPv6 address, voice chat may use the wrong source address when sending UDP packets to clients.
...
- Set `bind_address` in the server config to the IPv6 address (or domain) that has its ports opened (however, this will prevent IPv4-only players from using voice chat).
